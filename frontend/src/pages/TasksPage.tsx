...
export function TasksPage() {
  ...
  const isGlobal = (user?.role || '').toUpperCase() === 'GLOBAL_MANAGER';
  ...
  async function fetchTasks() {
    setLoading(true);
    try {
      const r = await fetch(`${API}/api/v1/tasks`, { headers });
  ...
  async function fetchCustomers() {
    try {
      const r = await fetch(`${API}/api/v1/customers`, { headers });
      const d = await r.json();
  ...
  async function handleSave() {
    ...
    try {
      if (editingTask) {
        await fetch(`${API}/api/v1/tasks/${editingTask.id}`, { method: 'PUT', headers, body: JSON.stringify(body) });
      } else {
        await fetch(`${API}/api/v1/tasks`, { method: 'POST', headers, body: JSON.stringify(body) });
      }
      setModalOpen(false);
      fetchTasks();
  ...
  async function handleDelete(task: Task) {
    ...
    try {
      await fetch(`${API}/api/v1/tasks/${task.id}`, { method: 'DELETE', headers });
      fetchTasks();
  ...
  const inputStyle: React.CSSProperties = {
    ...
    background: th.bg, color: th.text, fontSize: 13, fontFamily: "'Inter','Segoe UI',sans-serif",
    outline: 'none',
  };
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1400, margin: '0 auto', fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <div style={{
        ...
        {(['ACTIVE', 'COMPLETED', 'PAUSED', 'CANCELLED'] as const).map(s => {
          ...
          return (
            <div key={s} style={{
              padding: '10px 18px', borderRadius: 8, background: th.bgCard,
              border: `1px solid ${th.border}`, display: 'flex', alignItems: 'center', gap: 10,
            }}>
              <div style={{
                width: 10, height: 10, borderRadius: '50%', background: STATUS_COLORS[s],
              }} />
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          ...
          {Object.keys(STATUS_COLORS).map(s => (
            <option key={s} value={s}>{statusLbl[s]}</option>
          ))}
        </select>
      </div>
      ...
      {/* ── TABLE ── */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: 60, color: th.textDim, fontSize: 14 }}>
          {(PAGE_LABELS[lang] || PAGE_LABELS.de).noTasks || 'Loading...'}
        </div>
      ) : filtered.length === 0 ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: th.bgCard }}>
                <th style={thStyle(th)} />
                <th style={{ ...thStyle(th), cursor: 'pointer' }} onClick={() => toggleSort('code')}>
                  {lbl.code}{sortArrow('code')}
                </th>
                <th style={{ ...thStyle(th), cursor: 'pointer', textAlign: 'left' }} onClick={() => toggleSort('name')}>
                  {lbl.name}{sortArrow('name')}
                </th>
                <th style={{ ...thStyle(th), cursor: 'pointer', textAlign: 'left' }} onClick={() => toggleSort('customer')}>
                  {lbl.customer}{sortArrow('customer')}
                </th>
                <th style={thStyle(th)}>{lbl.type}</th>
                <th style={{ ...thStyle(th), cursor: 'pointer', textAlign: 'right' }} onClick={() => toggleSort('hours')}>
                  {lbl.hours}{sortArrow('hours')}
                </th>
                <th style={thStyle(th)}>{lbl.status}</th>
                {isManager && <th style={thStyle(th)}>{lbl.actions}</th>}
              </tr>
            </thead>
            <tbody>
              {filtered.map(task => (
                ...
                >
                  <td style={tdStyle(th)}>
                    <div style={{
                      ...
                      background: task.color || '#8B7355', margin: '0 auto',
                    }} />
                  </td>
                  <td style={{ ...tdStyle(th), fontWeight: 700, fontFamily: 'monospace', color: gold }}>
                    {task.code}
                  </td>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 24 }}>
              <button onClick={() => setModalOpen(false)} style={btnStyle(th.btnBg, th.text)}>
                {lbl.cancel}
              </button>
              <button onClick={handleSave} style={btnStyle(gold, '#fff')}>
                {lbl.save}
              </button>
            </div>
          </div>
...
function tdStyle(th: Record<string, string>): React.CSSProperties {
  return {
    padding: '10px 14px', textAlign: 'center', fontSize: 13, color: th.text,
...
function labelStyle(th: Record<string, string>): React.CSSProperties {
  return {
    display: 'block', fontSize: 11, fontWeight: 700, color: th.textDim,
